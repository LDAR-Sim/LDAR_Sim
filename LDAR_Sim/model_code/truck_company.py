...
class truck_company:
    ...
    def make_maps(self):
        ...
        output_raster = gdal.GetDriverByName('GTiff').Create('DD_truck_map_' + self.parameters['simulation'] + '.tif',
        ...
        output_raster = gdal.GetDriverByName('GTiff').Create('MCB_truck_map_' + self.parameters['simulation'] + '.tif',
                                                             ncols, nrows, 1, gdal.GDT_Float32)
        output_raster.SetGeoTransform(geotransform)  # Specify file coordinates
        srs = osr.SpatialReference()  # Establish coordinate encoding
        srs.ImportFromEPSG(4326)  # Specify WGS84 lat/long

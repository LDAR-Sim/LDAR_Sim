...
from initialization.versioning import (LEGACY_PARAMETER_WARNING,
...
from testing.unit_testing.test_initialization.test_input_manager.input_manager_testing_fixtures import (  # Noqa: 401
    ...
    mock_parameter_incorrect_maj_ver_fix, mock_parameter_incorrect_min_ver_fix,
...
def test_043_test_incorrect_minor_parameter_version(
        mocker,
        capsys,
        mock_parameter_incorrect_min_ver
):
    mock_old_params = False
    ...
    assert (captured.out == expected_print_str
            and input_mngr.old_params is True)


def test_043_test_incorrect_major_parameter_version(
        mocker,
        capsys,
        mock_parameter_incorrect_maj_ver
):
    mock_old_params = False
...
def test_043_test_only_major_parameter_version(
        mocker,
        capsys,
        mock_parameter_maj_only_ver
):
    mock_old_params = False

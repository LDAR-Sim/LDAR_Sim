...
# File:        campaign.methods
# Purpose:     Sets up campaigns for follow-up work practice
#
#
...
def setup_campaigns(campaigns, prog_params, virtual_world, n_sites, n_screening_rs_sets):
    ...
    for midx, rs in n_screening_rs_sets.items():
        ...
        if rs != "varies" or meth["follow_up"]["min_followup_type"] == "annual":
            ...
            min_FU_d_to_end = meth["follow_up"]["min_followup_days_to_end"]
            ...
            campaigns.update(
                {
                    midx: {
                        ...
                        "current_campaign": 0,
                        "schedule": [cnt * days_in_campaign for cnt in range(n_campaigns)],
...
def update_campaigns(campaigns, sites, cur_ts, current_date):
    for midx, cpgn in campaigns.items():
        ...
        if cpgn["current_campaign"] < cpgn["n_campaigns"] and cur_ts > next_camp_start:
            # Move to next Campaign
            campaigns[midx].update(
                {
                    "sites_followed_up": set([]),
                    "current_campaign": cpgn["current_campaign"] + 1,
                    "FU_check_ts": next_camp_start + (cpgn["n_days"] - cpgn["min_FU_check"]),
                }
            )
        if cur_ts == cpgn["FU_check_ts"]:
            # Move flag sites
            FU_sites = [s for s in sites if s["facility_ID"] in cpgn["sites_followed_up"]]
            Non_FU_sites = [s for s in sites if s["facility_ID"] not in cpgn["sites_followed_up"]]

...
class BaseCompany:
    ...
    def __init__(
        ...
        self.DD_map = np.zeros(
            (len(self.state["weather"].longitude), len(self.state["weather"].latitude))
        )
        ...
        for idx, cnt in enumerate(self.crews):
            self.timeseries["{}_cost".format(self.name)][
    ...
    def deploy_crews(self):
        ...
        if c_date.month in self.deployment_months and c_date.year in self.deployment_years:
            ...
            for idx in range(n_working_crews):
                ...
                crew_site_list = self.schedule.get_crew_site_list(
                    site_pool, idx, n_working_crews, self.crews
                )
                # Send crew to site
        ...
        for site in self.state["sites"]:
            ...
            if self.state["t"].current_date.day == 31 and self.state["t"].current_date.month == 12:
        ...
        if self.config["is_follow_up"]:
            self.state["flags"] = [
                site["facility_ID"] for site in self.state["sites"] if site["currently_flagged"]
            ]
        return
    ...
    def flag_sites(self):
        ...
        if self.state["t"].current_date.date() >= can_flag_date.date():
            ...
            if self.config["follow_up"]["interaction_priority"] == "proportion":
                prop_to_flag = int(math.ceil(len(site_wl) * self.config["follow_up"]["proportion"]))
                # site_wl is an ordered by leak size.
                if prop_to_flag > 0.0:
                    site_wl_subset = list(site_wl.values())[:prop_to_flag]
                    target_sites = [
                        site
                        for site in site_wl_subset
                        if site["site_measured_rate"] > self.config["follow_up"]["threshold"]
                    ]
            if self.config["follow_up"]["interaction_priority"] == "threshold":
                ...
                prop_to_flag = int(
                    math.ceil(len(sites_above_thresh) * self.config["follow_up"]["proportion"])
                )
    ...
    def site_reports(self):
        ...
        for site in self.state["sites"]:
            site["{}_prop_DDs".format(self.name)] = self.DD_map[
                site["lon_index"], site["lat_index"]
            ]
